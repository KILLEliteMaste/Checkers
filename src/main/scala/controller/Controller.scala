...
import controller.PlayerState.{PLAYER_1, PLAYER_2, PlayerState}
import model.{Cell, Field, FieldMatrix, Position}
import util.Observable

case object Controller extends Observable {
  var field: Field = Field(8)
  var gameState: GameState.Value = GameState.IDLE
  var playerState: PlayerState.Value = PLAYER_1
  ...
  def changePlayerTurn(): Unit = {
    if (playerState == PLAYER_1) playerState = PLAYER_2 else playerState = PLAYER_1
    notifyObservers()
  }
  ...
  def checkIfAllCellsBelongToPlayer(player: PlayerState, field: Field, positions: Vector[Position]): Boolean = {
    if (player == PLAYER_1) {
      for (elem <- positions) {
